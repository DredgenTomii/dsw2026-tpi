...
public class AvailabilityService : IAvailabilityService
{
    ...
    public async Task<IEnumerable<AvailabilityModel.SlotResponse>> GetByDoctor(
        Guid doctorId,
        DateOnly? from,
        DateOnly? to)
    {
    ...
    private async Task<AvailabilityModel.Response> GenerateMonth(
        ...
        bool isOverwrite)
    {
        ...
        if (!doctor.IsActive)
            throw new BusinessRuleException(
                "El médico no se encuentra activo",
                "DOCTOR_INACTIVE");
        ...
        if (rules.Count == 0)
            throw new ValidationException(
                ...
                "AVAILABILITY_EMPTY_RULES");

        ValidateBlocksOf30(rules);
        ValidateNoOverlaps(rules);

        var bookedSlots = await HandleExistingMonth(request, isOverwrite);

        var (rangeStart, lastOfMonth) =
            ResolveGenerationRange(request.Year, request.Month);

        var holidays =
            await _holidayProvider.GetHolidays(request.Year, request.Month);
        ...
        foreach (var dayRule in rules)
        {
            ...
            for (var date = rangeStart;
                 ...
                 date = date.AddDays(1))
            {
                if (date.DayOfWeek != dayRule.DayOfWeek)
                    continue;

                if (holidays.Contains(date))
                    continue;

                foreach (var (start, end) in SplitInSlotsOf30(
                             dayRule.StartTime,
                             dayRule.EndTime))
                {
    ...
    private async Task<List<AvailabilitySlot>> HandleExistingMonth(
        AvailabilityModel.Request request,
        bool isOverwrite)
    {
        var existingRules =
            (await _persistence.GetFiltered<AvailabilityRule>(r =>
    ...
    private static (DateOnly RangeStart, DateOnly LastOfMonth)
        ResolveGenerationRange(int year, int month)
    {
        ...
        if (rangeStart > lastOfMonth)
            throw new ValidationException(
                "El mes indicado ya finalizó",
                "AVAILABILITY_MONTH_PAST");

        return (rangeStart, lastOfMonth);
    }

    private static IEnumerable<(TimeOnly Start, TimeOnly End)>
        SplitInSlotsOf30(
            TimeOnly start,
            TimeOnly end)
    {
    ...
    private static void ValidateMonth(int year, int month)
    {
        if (month is < 1 or > 12)
            throw new ValidationException(
                ...
                "AVAILABILITY_INVALID_MONTH");

        if (year < DateTime.UtcNow.Year)
            throw new ValidationException(
                "El año indicado no es válido",
                "AVAILABILITY_INVALID_YEAR");
    }

    private static void ValidateBlocksOf30(
        List<AvailabilityModel.DayRuleRequest> rules)
    {
        foreach (var rule in rules)
        {
            ...
            var totalMinutes =
                (rule.EndTime.ToTimeSpan() -
                 rule.StartTime.ToTimeSpan()).TotalMinutes;
    ...
    private static AvailabilityModel.SlotResponse ToSlotResponse(
